    ...
    useEffect(() => {
        ...
        if (errorParam) {
            const errorMap: Record<string, string> = {
                google_cancelled: "Login com o Google foi cancelado.",
                google_error: "Falha na comunicação com o Google. Tente novamente.",
                google_not_configured: "Login com Google temporariamente indisponível no servidor.",
                google_email_unverified: "Seu e-mail no Google não está verificado.",
                google_auth_failed: "Não foi possível validar sua conta Google. Tente novamente.",
                google_state_invalid: "Sessão expirada. Por favor, tente novamente.",
                google_email_unavailable: "Não foi possível obter o e-mail da sua conta Google."
            };
            setAuthError(errorMap[errorParam] || "Ocorreu um erro ao entrar com o Google.");
            window.history.replaceState({}, document.title, window.location.pathname);
        }
                    ...
                    {!token && (
                        <>
                            <div className={styles.tabs}>
                                <button className={isLogin ? styles.activeTab : ""} onClick={() => { setIsLogin(true); setShowVerificationMessage(false); setAuthError(null); }}>ENTRAR</button>
                                <button className={!isLogin ? styles.activeTab : ""} onClick={() => { setIsLogin(false); setShowVerificationMessage(false); setAuthError(null); }}>CADASTRAR</button>
                            </div>
                            ...
                            {showVerificationMessage ? (
                                <div style={{ textAlign: 'center', padding: '30px 10px' }}>
                                    ...
                                    <button 
                                        ...
                                        onClick={() => { setIsLogin(true); setShowVerificationMessage(false); setAuthError(null); }}
                                    >
                                        IR PARA LOGIN
                                    </button>
                                </div>
